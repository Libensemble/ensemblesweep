...
def my_function(core, edge, x):
    # Simply sum the inputs after multiplying
    return float(edge) * np.sum(x)
...
if __name__ == "__main__":
    ...
    # do a single sample, run against the function/simulation. 
    # *This will update both the object with a single result, **and** an estimated time*
    sweep.run(1)
    print(sweep.results)
    ...
    # Results are also separated into individual files in the sweep directory